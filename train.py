...
import models
from test__ import asv_cal_accuracies, cal_roc_eer
import os
import sys
import time
...
if __name__ == '__main__':
    ...
    if not os.path.exists('./trained_models_time_orj/'):
        os.makedirs('./trained_models_time_orj/')
    ...
    if data_type == 'time_frame':
        if dataset == 15:
            ...
            eval_protocol_file_path = root_path + \
                'ASVspoof2019_LA_cm_protocols/ASVspoof2019.LA.cm.eval.trl.txt'
            train_data_path = 'data/train_6/'
            dev_data_path =  'data/dev_6/'
            eval_data_path =  'data/eval_6/'
        ...
        if dataset == 15:
            ...
            eval_protocol_file_path = root_path + \
                'ASVspoof2019_LA_cm_protocols/ASVspoof2019.LA.cm.eval.trl.txt'
            train_data_path =  'data/train_6.4_cqt/'
            dev_data_path =  'data/dev_6.4_cqt/'
            eval_data_path =  'data/eval_6.4_cqt/'
    else:
    ...
    num_total_learnable_params = sum(i.numel()
                                     for i in Net.parameters() if i.requires_grad)
    ...
    # TODO: Training
    print('Training data: {}, Date type: {}. Training started...'.format(
    ...
    if not os.path.exists('./trained_models_time_orj/train_log/'):
        os.makedirs('./trained_models_time_orj/train_log/')

    log_path = './train_models_time_orj/train_log/'
    ...
    for epoch in range(num_epoch):
        ...
        for batch in train_loader:
            ...
            if loss_type == 'mixup':
                ...
                loss = lam * \
                    F.cross_entropy(preds, labels) + (1 - lam) * \
        ...
        dev_accuracy, d_probs = asv_cal_accuracies(
            dev_protocol_file_path, dev_data_path, Net, device, data_type=data_type, dataset=dataset)
        ...
        if d_eer <= best_d_eer[0]:
            ...
            eval_accuracy, e_probs = asv_cal_accuracies(
                eval_protocol_file_path, eval_data_path, Net, device, data_type=data_type, dataset=dataset)
            e_eer = cal_roc_eer(e_probs, show_plot=False)
        else:
            e_eer = .99
            eval_accuracy = 0.00

        net_str =  str(epoch) + '.pth'
        torch.save({'epoch': epoch, 'model_state_dict': Net.state_dict(),
                    'optimizer_state_dict': optimizer.state_dict(),
                    'scheduler_state_dict': scheduler.state_dict(),
                    'loss': loss_per_epoch}, ('./train_models_time_orj/' + net_str))
        ...
        df.to_csv(log_path + time_name + '.csv', sep=' ',
